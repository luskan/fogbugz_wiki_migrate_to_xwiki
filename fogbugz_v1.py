...
class FogBugz:
    def __init__(self, url, token=None, api_version=8, auth_user=None, auth_password=None):
        self.__handlerCache = {}
        if not url.endswith('/'):
            url += '/'
        ...
        if auth_user and auth_password:
            ...
            passman.add_password(None, url, auth_user, auth_password)
            authhandler = urllib_request.HTTPBasicAuthHandler(passman)
            self._opener = urllib_request.build_opener(authhandler)
            urllib_request.install_opener(self._opener)
        else:
